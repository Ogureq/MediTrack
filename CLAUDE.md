...
- **Models** (`Gemocode/Models/Models.swift`): all 12 `@Model` classes in one file (MedicalReport, LabResult, ReportAttachment, VitalSample, Medication, HealthProfile, ScoreSnapshot, SymptomEntry, Appointment, HealthGoal, Reminder, ReminderCompletion). Enums are stored as raw strings. A new model must also be registered in the `Schema` in `GemocodeApp.swift`, in `SampleData.eraseAllData`, in `BackupService` (backup payload fields are **optional** for backward compatibility — never add a required field), and in the in-memory `ModelContainer` lists in tests.
