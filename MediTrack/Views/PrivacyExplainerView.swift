...
/// A calm, read-only explainer of MediTrack's on-device-only data model.
///
...
struct PrivacyExplainerView: View {
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                ...
                section(title: "What's Stored", systemImage: "internaldrive", tint: .teal) {
                    ...
                    bullet("There's no MediTrack account, server, or sync. Nothing is uploaded automatically — ever.")
                }
                ...
                section(title: "What's Protected", systemImage: "lock.shield", tint: .blue) {
                    ...
                    bullet("Backups you export are encrypted with AES-GCM using a key derived from a passphrase only you know. MediTrack can't open a backup without it, and neither can anyone else.")
                }

                section(title: "What Leaves the Device — Only When You Ask", systemImage: "arrow.up.right.square", tint: .orange) {
                    bullet("The AI Summary feature in Profile & Settings is opt-in and off by default. When you tap Generate, MediTrack sends the health review it already built for you — your score, findings, and trends, including the specific lab and vital values behind them — to Anthropic's API, using your own API key.")
                    bullet("Your name, date of birth, blood type, allergies, Medical ID, and original documents are never included in that request.")
                    bullet("This is the only network call MediTrack ever makes. Leave the API key blank to keep the app fully offline.")
                }
                ...
                section(title: "What Never Leaves", systemImage: "shield.lefthalf.filled", tint: .green) {
                    ...
                    bullet("Your Medical ID is for on-device viewing. It only leaves your phone if you personally tap Share on that screen or export a backup — MediTrack never sends it on its own.")
                }
                ...
                Text("This screen describes MediTrack's current behavior. If that ever changes, this page — and the disclaimers throughout the app — change with it.")
                    .font(.caption)
    ...
    private var heroCard: some View {
        VStack(spacing: 10) {
            ...
            Text("MediTrack is local-first: everything you enter is stored on this device, not in the cloud.")
                .font(.subheadline)
