...
- **Vitals tracking** — Weight, blood pressure, resting heart rate, blood glucose, SpO2, body temperature, respiratory rate (typical 12–20 breaths/min), and sleep (7–9 h), with Swift Charts visualizations that include healthy-range bands.
...
| `MediTrack/Views/` | `Dashboard`, `Reports` (list/detail/add), `Review`, `Trends`, `Vitals`, `Symptoms`, `Medications`, `Appointments`, `MedicalID`, `Profile`, the onboarding flow, and the root `TabView`. |
