...
- **Quarterly Review** — Every 90 days the Dashboard invites you to a recap of your quarter: score trajectory, what changed in your vitals and labs (judged conservatively — only medically unambiguous metrics are labeled improved or worsened), your streak and goal wins, and questions worth bringing to your doctor. Computed entirely on-device from your own data and shareable as plain text.
- **Biomarker carousel** — A horizontally scrolling Dashboard row with a card per lab test you track: latest value, status against the sex-specific reference range, and a mini history sparkline, tapping through to the full lab detail screen.
...
- **Documents library** — More → Documents gathers every photo and PDF attached to any report into one searchable grid: thumbnails, category filter chips, and search across filenames and report titles, opening the same full-screen viewer as the report detail screen. Everything stays on-device.
...
- **Home-screen widget** — A Health Score widget (small & medium) showing your latest score ring, headline, and recent vitals via a shared app group; the health data renders redacted while the device is locked.
...
- **Quick Add** — Type one natural sentence on the Dashboard ("aspirin 100mg twice daily", "bp 128/82", "dentist tomorrow 3pm") and an on-device parser turns it into a live-previewed medication, vital, symptom, appointment, or reminder — confirm to save. If the offline parser can't read a sentence and an API key is configured, an optional "Fill with AI" button interprets it instead, with every value re-validated on-device and the result clearly labeled for double-checking.
...
Gemocode uses an editorial, paper-and-ink design system built entirely with native SwiftUI — no third-party UI libraries. Flat canvas surfaces carry hairline-ruled cards and ledger rows with micro-label section headers; every lab value across the app renders in the same grammar of name, value, status tag, and segmented reference-range bar. The system lives in a small token set in `Gemocode/Support/Theme.swift` (the `Editorial` palette: canvas, ink, muted, hairlines, status-tag and range-zone colors, in WCAG AA-checked light and dark variants — light is the default) and a component kit in `Support/EditorialComponents.swift` (`RangeBar`, `EditorialTag`, `MicroLabel`, ledger rows, pill button styles, and the text `PillTabBar` navigation). Type is the system SF face with tight tracking. The app icon carries a teal-to-blue gradient with a white medical cross and pulse line. Trends charts support touch scrubbing with a value tooltip, and key actions such as saves are confirmed with haptic feedback.
