...
enum HealthKitService {
    ...
    /// launch (see `GemocodeApp`) — it is a cheap no-op when the toggle is
    /// off.
    ///
    ...
    /// device, HealthKit only wakes a *suspended* Gemocode process for new
    ...
    /// background delivery so HealthKit stops waking Gemocode for this
